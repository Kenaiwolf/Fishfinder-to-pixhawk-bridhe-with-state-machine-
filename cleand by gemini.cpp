...
#define RMB_TIMEOUT_MS 3000UL //can be lowerd for helix dont change for 598ci HD
...
static bool circleApproachThetaInit=false;     // sentinel: not yet sampled this approach  
...
//    {
//        char dbgDt[24];
//        snprintf_P(dbgDt,sizeof(dbgDt),PSTR("DPT dt=%lu ms"),(unsigned long)(millis()-lastDPTms));
//        sendStatusText(MAV_SEVERITY_DEBUG,dbgDt);
//    }
//#endif
    lastDPTms=millis();
    if(accept){
...
static void setCircleAnchor(float lat,int32_t lat_int,int32_t lon_int,float headingDeg){
    ...
    circleApproachThetaInit=false;   // sentinel: "not yet sampled"
...
static void enterContourFollow(){
    ...
    float entryHeadingDeg=haveLastLegHeading?lastLegHeadingDeg:(havePixhawkHeading?pixhawkHeadingDeg:vtg_cog_deg);
    ...
    while(Serial.available()&&maxBytesPerCall--){
        ...
        if(mavlink_parse_char(MAVLINK_COMM_0,c,&mavMsg,&mavRxStatus)){
            ...
            if(mavMsg.msgid==MAVLINK_MSG_ID_HEARTBEAT){
                ...
                hbRxCount++;
                if(mavMsg.sysid==TARGET_SYS){
                    mavlink_heartbeat_t hb;
                    mavlink_msg_heartbeat_decode(&mavMsg,&hb);
                    uint8_t newPixhawkMode=(uint8_t)hb.custom_mode;
                    if(newPixhawkMode==ROVER_MODE_GUIDED&&pixhawkMode!=ROVER_MODE_GUIDED&&
            ...
            if(mavMsg.msgid==MAVLINK_MSG_ID_RC_CHANNELS_RAW){
                ...
                static_assert(SPEED_TRIM_CHANNEL>=1&&SPEED_TRIM_CHANNEL<=8,
                              "SPEED_TRIM_CHANNEL must be 1-8");
                static_assert(offsetof(mavlink_rc_channels_raw_t,chan2_raw)-
                              offsetof(mavlink_rc_channels_raw_t,chan1_raw)==sizeof(uint16_t),
                              "RC channel fields are not contiguous — use switch instead");
                static_assert(offsetof(mavlink_rc_channels_raw_t,chan8_raw)-
                              offsetof(mavlink_rc_channels_raw_t,chan1_raw)==7u*sizeof(uint16_t),
                              "RC channel fields are not contiguous — use switch instead");
                ...
                if(trimState!=prevTrimState){
                    ...
                    if(trimState!=0&&activeSpeedSlot>=0&&((navState==NavState::GUIDED&&rmbActive&&(millis()-lastRMBms)<RMB_TIMEOUT_MS)||navState==NavState::CONTOUR_FOLLOW)){
                        ...
                        speedDirty=true;speedChangedMs=millis();
#if DEBUG_SPEED_TRIM
                        {
                            char dbg[40];
                            char spdStr[8];
                            dtostrf(spd,4,2,spdStr);
                            snprintf(dbg,sizeof(dbg),"TRIM st=%u slot=%d spd=%s",
                                     (unsigned)trimState,(int)activeSpeedSlot,spdStr);
                            sendStatusText(MAV_SEVERITY_DEBUG,dbg);
                        }
#endif
...
void loop(){
    ...
    if(crashLoopMode){  
        ...
        if(millis()-lastCrashDumpMs>=3000UL){  
            ...
            char dbg[64];  
			snprintf(dbg,sizeof(dbg),"CRASHLOOP bc=%u minram=%d",  
			(unsigned)lastBreadcrumb,(int)lastMinRam);  
            ...
            if (rmbFresh) {
                if (pixhawkModeKnown && pixhawkMode == ROVER_MODE_GUIDED) {
                    ...
                    if (contourTrigger && (!haveDPT || !haveVTGcog)) {
                        if(!posTargetSentForDest){
                            if(sendPositionTarget(rmb_lat, rmb_lon)){
                                posTargetSentForDest=true;
                            }
                        }
                    } else {
                        float commandedSpeed = vescNotActive ? 0.0f :
                                                (activeSpeedSlot==0)?travelSpeed:(activeSpeedSlot==1)?fishingSpeed:DEFAULT_SPEED_MS;
                        if(speedNeedsResend){
                            ...
                            if(speedChanged||(now-lastSpeedSendMs>=SPEED_KEEPALIVE_MS)){
                                if(sendSpeedCommand(commandedSpeed)){
#if DEBUG_SPEED_TRIM
                                    {
                                        ...
                                        snprintf(dbg,sizeof(dbg),"TRIM chg=%d cs=%s ls=%s",
                                                 (int)speedChanged,csStr,lsStr);
                                        sendStatusText(MAV_SEVERITY_DEBUG,dbg);
                                    }
#endif
                                    lastSpeedSendMs=now;
                ...
                } else if (pixhawkModeKnown && pixhawkMode != ROVER_MODE_GUIDED) {
...
// === SEC:LOOP_CONTOUR_TOP BEGIN ===
            ...
            }else if(pixhawkMode!=ROVER_MODE_HOLD && pixhawkMode!=ROVER_MODE_GUIDED){
...
// === SEC:LOOP_CONTOUR_APPROACH BEGIN ===
                ...
                if(haveDPT&&dpt_depth_m<CONTOUR_SAFETY_DEPTH_M){  
                    ...
                    if(!circleApproachThetaInit){  
                        circleApproachPrevTheta=theta;  
                        circleApproachAccumAngle=0.0f;  
                        circleApproachThetaInit=true;  
                    }else{  
                    ...
                    if(fabsf(circleApproachAccumAngle)>=180.0f){  
                        ...
                        float bearingToArcDeg=atan2f(-dE,-dN)*57.29577951f;  
                        if(bearingToArcDeg<0.0f)bearingToArcDeg+=360.0f;  
                        sendVelocityTarget(bearingToArcDeg, activeSpeedSlot==1?fishingSpeed:travelSpeed);  
                    }  
                }else{  
                        ...
                        if(crossed){
                            ...
                            if(depthSlopeCount>=2){
                                ...
                                if(distSpan>=DEPTH_SLOPE_MIN_DIST_M){
                                    ...
                                    if(fabsf(slope)>=DEPTH_SLOPE_CONF_THR){
                                        deepSide=(slope>0.0f);
                                        deepSideValid=true;
                                    }
                                }
...
// PARITY tag=LOOP_CONTOUR_SURVEY_A toklen=1420 braces={15,11} parens={28,28} semi=29 edges={elseif(c,rue;}}}}}
// === SEC:LOOP_CONTOUR_SURVEY_B BEGIN ===
                            float slopeV=0.0f;
                            if(depthSlopeCount>=2){
                                uint8_t oldIdx=(depthSlopeHead+DEPTH_SLOPE_SAMPLES-depthSlopeCount)%DEPTH_SLOPE_SAMPLES;
								float distSpan=depthSlopeCumDist-(depthSlopeBufDist[oldIdx]*0.01f);
                                if(distSpan>0.01f) slopeV=(dpt_depth_m-(depthSlopeBufDepth[oldIdx]*0.01f))/distSpan;
                            }
#if DEBUG_CONTOUR_TUNING
                            {
                                char dbg[64];
                                char peStr[8],ceStr[8],slStr[8];
                                dtostrf(prevErr,4,2,peStr);
                                dtostrf(currErr,4,2,ceStr);
                                dtostrf(slopeV,5,3,slStr);
                                snprintf_P(dbg,sizeof(dbg),
                                           PSTR("XING pe=%s ce=%s slV=%s cnt=%d"),
                                           peStr,ceStr,slStr,(int)depthSlopeCount);
                                sendStatusText(MAV_SEVERITY_DEBUG,dbg);
                            }
#endif
                            if(deepSideValid){
                                float dev=fabsf(currErr);
                                if(dev<circleBestDev){
                                    circleBestDev=dev;
                                    float h=theta+90.0f;
                                    if(h>=360.0f)h-=360.0f;
                                    circleBestHeading=h;
                                    circleBestLat_int=currentLat_int;
                                    circleBestLon_int=currentLon_int;
                                    circleBestDeepRight=deepSide;
                                    circleCrossFound=true;
#if DEBUG_CONTOUR_TUNING
                                    {
                                        char dbg2[64];
                                        char hdStr[8],dvStr[8];
                                        dtostrf(circleBestHeading,4,1,hdStr);
                                        dtostrf(circleBestDev,4,2,dvStr);
                                        snprintf_P(dbg2,sizeof(dbg2),
                                                   PSTR("XING acc! hd=%s dev=%s side=%c"),
                                                   hdStr,dvStr,circleBestDeepRight?'R':'L');
                                        sendStatusText(MAV_SEVERITY_DEBUG,dbg2);
                                    }
#endif
                                }
                            }
                    ...
                    if(fabsf(circleAccumAngle)>=360.0f){
                        if(!circleCrossFound){
                            EEPROM.update(EEPROM_ADDR_BREADCRUMB,BC_SURVEY_ENTER);
                            circleAccumAngle=0.0f;
                            depthSlopeHead=0;
                            depthSlopeCount=0;
                            depthSlopeCumDist=0.0f;
#if DEBUG_SURVEY || DEBUG_APPROACH
							sendStatusText_P(MAV_SEVERITY_WARNING,PSTR("SURVEY: lap done, no XING -> retry lap"));
#endif
                        }else{
                            EEPROM.update(EEPROM_ADDR_BREADCRUMB,BC_FOLLOW_ENTER);
                            contourPhase=ContourPhase::FOLLOW;
                            contourHeadingDeg=circleBestHeading;
                            contourBaseHeadingDeg=contourHeadingDeg;
                            contourKpSigned=circleBestDeepRight?1.0f:-1.0f;
                            contourTrendRefSet=false;
                            followPrevError_m=circleBestDev;
                            lastContourOffsetDeg=0.0f;
                            circleAccumAngle=0.0f;
#if DEBUG_FOLLOW || DEBUG_SURVEY
                        ...
                        float radErr=CONTOUR_CIRCLE_RADIUS_M-dist;
                        float offsetDeg=radErr*CIRCLE_RADIUS_GAIN;
                        if(offsetDeg>CIRCLE_RADIUS_MAX_CORR_DEG)offsetDeg=CIRCLE_RADIUS_MAX_CORR_DEG;
                        if(offsetDeg<-CIRCLE_RADIUS_MAX_CORR_DEG)offsetDeg=-CIRCLE_RADIUS_MAX_CORR_DEG;
                        float dir=theta+90.0f+offsetDeg;
                        if(dir>=360.0f)dir-=360.0f;
                        sendVelocityTarget(dir, activeSpeedSlot==1?fishingSpeed:travelSpeed);
                    }
                    if(posIsNew){
                        circlePrevLat_int=currentLat_int;
                        circlePrevLon_int=currentLon_int;
...
// PARITY tag=LOOP_CONTOUR_SURVEY_B toklen=1506 braces={14,18} parens={22,22} semi=33 edges={float,peed);}}}
// === SEC:LOOP_CONTOUR_FOLLOW BEGIN ===
            else if(contourPhase==ContourPhase::FOLLOW){
                EEPROM.update(EEPROM_ADDR_BREADCRUMB,BC_FOLLOW_LOOP_TOP);
                float currDepth=haveDptEma?dpt_depth_ema_m:dpt_depth_m;
                if(haveDPT&&currDepth<CONTOUR_SAFETY_DEPTH_M){
                    gotoLoiterReset(BC_LOITER_WP_RESET_FOLLOW);
                    sendStatusText_P(MAV_SEVERITY_WARNING,PSTR("CONTOUR: shallow depth -> LOITER"));
                }else if(haveCurrentPos){
                    float err_m=contourTargetDepth_m-currDepth;
                    bool errSmall=fabsf(err_m)<=CONTOUR_CORRECT_DEPTH_M;
                    if(errSmall){
                    ...
                    if(haveDPT){
                        if(errSmall&&!contourTrendRefSet){
                            contourTrendRefLat_int=currentLat_int;
                            contourTrendRefLon_int=currentLon_int;
                            contourTrendRefDepth=currDepth;
                            contourTrendRefCosLat=cosf(currentLat_deg*(float)M_PI/180.0f);
                            contourTrendRefSet=true;
                        }else if(errSmall&&contourTrendRefSet){
                            float dN=(currentLat_int-contourTrendRefLat_int)*METERS_PER_INT7_LAT;
                            float dE=(currentLon_int-contourTrendRefLat_int)*METERS_PER_INT7_LAT*contourTrendRefCosLat;
                            float dist=sqrtf(dN*dN+dE*dE);
                            if(dist>=CONTOUR_TREND_DIST_M){
                                float depthDelta=currDepth-contourTrendRefDepth;
                                float dAng=0.0f;
                                if(fabsf(depthDelta)>TREND_DEPTH_MIN_M){
                                    float grade=depthDelta/dist;
                                    dAng=grade*TREND_NUDGE_GAIN*contourKpSigned*57.29577951f;
                                    if(dAng>TREND_NUDGE_MAX_DEG)dAng=TREND_NUDGE_MAX_DEG;
                                    if(dAng<-TREND_NUDGE_MAX_DEG)dAng=-TREND_NUDGE_MAX_DEG;
                                }
                                contourBaseHeadingDeg+=dAng;
                                if(contourBaseHeadingDeg>=360.0f)contourBaseHeadingDeg-=360.0f;
                                if(contourBaseHeadingDeg<0.0f)contourBaseHeadingDeg+=360.0f;
                                contourTrendRefLat_int=currentLat_int;
                                contourTrendRefLon_int=currentLon_int;
                                contourTrendRefDepth=currDepth;
                                contourTrendRefCosLat=cosf(currentLat_deg*(float)M_PI/180.0f);
                    ...
                    if(!errSmall&&haveDPT){
                        if((err_m>0.0f&&followPrevError_m<0.0f)||(err_m<0.0f&&followPrevError_m>0.0f)){
                            if((now-lastSideFlipMs)>CONTOUR_FLIP_GATE_MS){
                                contourKpSigned=-contourKpSigned;
                                lastSideFlipMs=now;
#if DEBUG_FOLLOW || DEBUG_SURVEY
								sendStatusText_P(MAV_SEVERITY_WARNING,PSTR("CONTOUR: side-flip detected"));
#endif
                            }else if(fabsf(err_m)>CONTOUR_FLIP_ABORT_M){
                                EEPROM.update(EEPROM_ADDR_BREADCRUMB,BC_FOLLOW_REENTER);
                                reenterContourFromLastGood();
                            }
                        }
                    }
                    if(haveDPT)followPrevError_m=err_m;
                    float offsetDeg=0.0f;
                    if(haveDPT){
                        if(errSmall)offsetDeg=0.0f;
                        else{
                            float effErr=err_m-(err_m>0.0f?CONTOUR_CORRECT_DEPTH_M:-CONTOUR_CORRECT_DEPTH_M);
                            offsetDeg=effErr*CONTOUR_GAIN_DEG_PER_M*contourKpSigned;
                            if(offsetDeg>CONTOUR_MAX_OFFSET_DEG)offsetDeg=CONTOUR_MAX_OFFSET_DEG;
                            if(offsetDeg<-CONTOUR_MAX_OFFSET_DEG)offsetDeg=-CONTOUR_MAX_OFFSET_DEG;
                        }
                        lastContourOffsetDeg=offsetDeg;
                    }else{
                        offsetDeg=lastContourOffsetDeg;
                    }
                    contourHeadingDeg=contourBaseHeadingDeg+offsetDeg;
                    if(contourHeadingDeg>=360.0f)contourHeadingDeg-=360.0f;
                    if(contourHeadingDeg<0.0f)contourHeadingDeg+=360.0f;
                    EEPROM.update(EEPROM_ADDR_BREADCRUMB,BC_FOLLOW_HEADING_DONE);
                    float dN_LG=(currentLat_int-contourLastGoodLat_int)*METERS_PER_INT7_LAT;
                    float dE_LG=(currentLon_int-contourLastGoodLon_int)*METERS_PER_INT7_LAT*activationCosLat;
                    float distLG=sqrtf(dN_LG*dN_LG+dE_LG*dE_LG);
                    if((now-contourLastGoodMs)>CONTOUR_LOST_TIMEOUT_MS||distLG>CONTOUR_LOST_DIST_M){
                        EEPROM.update(EEPROM_ADDR_BREADCRUMB,BC_FOLLOW_REENTER);
                        reenterContourFromLastGood();
                    }else{
                        sendVelocityTarget(contourHeadingDeg, activeSpeedSlot==1?fishingSpeed:travelSpeed);
...
// === SEC:LOOP_CONTOUR_FOLLOW END ===
// PARITY tag=LOOP_CONTOUR_FOLLOW toklen=1561 braces={18,18} parens={37,37} semi=49 edges={elseif(c,peed);}}}}
        }
    }
}
